...
export class Report extends React.Component {
    ...
    insertLogForInsert(datalist,id){
       let self=this;
       let counter =0;
        if(datalist){
            let temp= new Promise((resolve,reject)=>{
                ...
                Object.keys(datalist).map(col=>{
                   ...
                   tempresult.push(  logging(logData));
                  
                }
                Promise.race(tempresult).then(function(values) {
                    console.log(values);
                  }).then(()=>{
                      if(counter === 0)
                     {self.setState({idchanged:id, ischanged :self.state.ischanged ? false :true});
                    counter++;
                    }
                  });
        ...
        axios({
            ...
            .then(function (response) {
                if (response.status === 200) {
                    ...
                    currentInstance.insertLogForInsert(InserData,id);
                }
            })
                ...
                >
                    ...
                    <Modal.Body>
                        ...
                        <input type="text" name="new_CardID"
                           disabled        value={this.props.cardNumber} onChange={this.handleChange.bind(this)} /><br/>
